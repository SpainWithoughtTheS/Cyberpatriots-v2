...
# Privilege check at the start of the script
...
def check_services():
    ...
    if not run_command("service --status-all | grep -E '\\[ \\+ \\]|\\[ - \\]' > installed_services.txt"):
        logging.error("Failed to check installed services.")
...
def check_weak_passwords():
    ...
    try:
        ...
        logging.error(f"Error checking weak passwords: {e}")
...
def check_file_system_integrity():
    ...
    if run_command("sudo umount /dev/sda1"):
        run_command("sudo fsck -f /dev/sda1")

# Main menu
def main():
    check_privileges()  # Ensure script is run with root privileges
    while True:
